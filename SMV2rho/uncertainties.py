...
def rho_err(z, bulk_rho, q0, dq0, qm, dqm, hr, dhr, dvdT, ddvdT, alpha0, dalpha0, 
    ...
    # take absolute value to avoid negative drawn (not ideal but doesn't matter 
    # too much)
    ...
    dens_params = np.loadtxt(dens_parameters)

    # discretise z (convert to metres)
    z_arr = np.linspace(0, z*1000, z_slices)

    # generate family of geotherms by sampling distributions
    Tz_all = np.array([cont_geotherm_heat_flux_difference(z_arr, T0=10, qm=j, q0=i, k=2.5, hr=k) 
                        for i, j, k in zip(q0_gauss, qm_gauss, hr_gauss)])
    T_errors = np.column_stack((np.mean(Tz_all, axis=0), np.std(Tz_all, axis=0)))

    # calculate v correction error using random family of 
    # geotherms and dvdT estimates
    v_err_all = Tz_all * np.transpose(np.repeat([dvdT_gauss], z_slices, axis=0))
    ...
    rho_0_error = (V2rho([9.81 * bulk_rho * z_arr/1000, vc_err[:,1]], *dens_params) -
                           V2rho([9.81 * bulk_rho * z_arr/1000, 0], *dens_params))
    rho_0_mean = (V2rho([9.81 * bulk_rho * z_arr/1000, vc_err[:,0]], *dens_params) -
                           V2rho([9.81 * bulk_rho * z_arr/1000, 0], *dens_params))
    ...
    thermal_expans_profiles = rho_thermal2(bulk_rho, np.transpose(Tz_all),
                                           alpha0=np.repeat([alpha0_gauss], 
                                                            z_slices, axis=0),
                                           alpha1=np.repeat([alpha1_gauss], 
                                                            z_slices, axis=0))[2]
    thermal_expans_profiles_frac = rho_thermal2(bulk_rho, np.transpose(Tz_all),
                                           alpha0=np.repeat([alpha0_gauss], 
                                                            z_slices, axis=0),
                                           alpha1=np.repeat([alpha1_gauss], 
                                                            z_slices, axis=0))[1]
    
    thermal_expans_abs_err = np.column_stack((np.mean(thermal_expans_profiles, axis=1), 
                                          np.std(thermal_expans_profiles, axis=1)))
    thermal_expans_frac_err = np.column_stack((np.mean(thermal_expans_profiles_frac , axis=1), 
    ...
    compress_profiles = compressibility(bulk_rho, Pz, K=np.repeat([K_gauss], 
                                                                  z_slices, axis=0))[2]
    compress_profiles_frac = compressibility(bulk_rho, Pz, K=np.repeat([K_gauss], 
                                                                       z_slices, axis=0))[1]
    
    compress_abs_err = np.column_stack((np.mean(compress_profiles, axis=1), 
                                    np.std(compress_profiles, axis=1)))
    compress_frac_err = np.column_stack((np.mean(compress_profiles_frac, axis=1), 
                                    np.std(compress_profiles_frac, axis=1)))
    
    # combine errors assuming they are uncorrelated (probably not completely correct for 
    # velocity correction and thermal expansion as they both depend on particular 
    # geotherm but a good approximation)

    # error and correction by adding terms
    total_correction_add = rho_0_errors[:,0] + thermal_expans_abs_err[:,0] + compress_abs_err[:,0]
    error_total_add = np.sqrt(rho_0_errors[:,1]**2 + thermal_expans_abs_err[:,1]**2 + 
                              compress_abs_err[:,1]**2)
    
    # error and correction by multiplication (probably the "proper" way to do it)
    # fractional change in rho_0
    compress_therm_frac = compress_frac_err[:,0] * thermal_expans_frac_err[:,0]
    # corrected mean rho(z)
    total_correction_frac = (bulk_rho + rho_0_errors[:,0]) * (compress_therm_frac) - bulk_rho
    # uncertainty in density correction as function of z
    error_total_frac = np.sqrt((rho_0_errors[:,1] * compress_therm_frac)**2 +
                               (compress_frac_err[:,1] * (bulk_rho + rho_0_errors[:,0]) * 
                                compress_therm_frac)**2 +
                               (thermal_expans_frac_err[:,1] * (bulk_rho + rho_0_errors[:,0]) * 
                                compress_therm_frac)**2)
    ...
    if make_plots is True:    
        ...
        fig.text(0.06, 0.5, 'Depth (km)', ha='center', va='center', rotation='vertical', fontsize=12)

        # Plotting

        # Panel 1
        axs[0, 0].plot(total_correction_add, -z_arr/1000, label='Average')
        axs[0, 0].plot(total_correction_add + error_total_add, -z_arr/1000, label=r'Total Corr $+ 1\sigma$')
        axs[0, 0].plot(total_correction_add - error_total_add, -z_arr/1000, label=r'Total Corr $- 1\sigma$')
        axs[0, 0].set_title('Total density error \nby addition')

        # Panel 2
        axs[0, 1].plot(total_correction_frac, -z_arr/1000, label='Average')
        axs[0, 1].plot(total_correction_frac + error_total_frac, -z_arr/1000, label=r'$+ 1\sigma$')
        axs[0, 1].plot(total_correction_frac - error_total_frac, -z_arr/1000, label=r'$- 1\sigma$')
        axs[0, 1].set_title('Total fractional \nerror')

        # Panel 3
        axs[0, 2].hist2d(np.concatenate(Tz_out)[:,1], -np.concatenate(Tz_out)[:,0], bins=25)
        axs[0, 2].plot(T_errors[:,0], -z_arr/1000, label='Average')
        axs[0, 2].plot(T_errors[:,0] + T_errors[:,1], -z_arr/1000, label=r'$+ 1\sigma$')
        axs[0, 2].plot(T_errors[:,0] - T_errors[:,1], -z_arr/1000, label=r'$- 1\sigma$')
        axs[0, 2].set_title('T error')

        # Panel 4
        axs[0, 3].plot(compress_abs_err[:,0], -z_arr/1000, label='Average')
        axs[0, 3].plot(compress_abs_err[:,0] + compress_abs_err[:,1], -z_arr/1000, label=r'$+ 1\sigma$')
        axs[0, 3].plot(compress_abs_err[:,0] - compress_abs_err[:,1], -z_arr/1000, label=r'$- 1\sigma$')
        axs[0, 3].set_title('Compression \nabsolute error')

        # Panel 5
        axs[1, 0].plot(thermal_expans_abs_err[:,0], -z_arr/1000, label='Average')
        axs[1, 0].plot(thermal_expans_abs_err[:,0] + thermal_expans_abs_err[:,1], -z_arr/1000, label=r'$+ 1\sigma$')
        axs[1, 0].plot(thermal_expans_abs_err[:,0] - thermal_expans_abs_err[:,1], -z_arr/1000, label=r'$- 1\sigma$')
        axs[1, 0].set_title('Thermal expansion \nabsolute error')

        # Panel 6
        axs[1, 1].plot(compress_frac_err[:,0], -z_arr/1000, label='Average')
        axs[1, 1].plot(compress_frac_err[:,0] + compress_frac_err[:,1], -z_arr/1000, label=r'$+ 1\sigma$')
        axs[1, 1].plot(compress_frac_err[:,0] - compress_frac_err[:,1], -z_arr/1000, label=r'$- 1\sigma$')
        axs[1, 1].set_title('Compression \nfractional error')

        # Panel 7
        axs[1, 2].plot(thermal_expans_frac_err[:,0], -z_arr/1000, label='Average')
        axs[1, 2].plot(thermal_expans_frac_err[:,0] + thermal_expans_frac_err[:,1], -z_arr/1000, label=r'$+ 1\sigma$')
        axs[1, 2].plot(thermal_expans_frac_err[:,0] - thermal_expans_frac_err[:,1], -z_arr/1000, label=r'$- 1\sigma$')
        axs[1, 2].set_title('Thermal expansion \nfractional error')

        # Panel 8
        axs[1, 3].plot(rho_0_errors[:,0], -z_arr/1000, label='Average')
        axs[1, 3].plot(rho_0_errors[:,0] - rho_0_errors[:,1], -z_arr/1000, label=r'$+ 1\sigma$')
        axs[1, 3].plot(rho_0_errors[:,0] + rho_0_errors[:,1], -z_arr/1000, label=r'$- 1\sigma$')
        axs[1, 3].set_title(r"Absolute $\rho(z)$ error")
        ...
        axs[1, 3].legend(legend_labels, loc='center', bbox_to_anchor=(0.2, -0.3), ncol=3)

        # Show the plot
        plt.show()
    
    error_average_add = np.sqrt(np.sum(error_total_add**2)/(len(error_total_add)-1))
    error_average_frac = np.sqrt(np.sum(error_total_frac**2)/(len(error_total_frac)-1))
