...
def plot_panels(
    ...
    ----------
    data : list of lists
        A list where each element represents a separate plot. Each element 
    ...
    plot_type : str, optional
        Type of plot to create ('scatter' or 'line'). Default is 'scatter'.
    cmap : str, optional
        Color map for scatter plot. Default is 'YlGnBu'.
    titles : list or None, optional
        Titles for each panel. Can be a list of strings or None to omit 
        titles.
    xlabels : list or None, optional
        X-axis labels for each panel. Can be a list of strings or None to 
        omit x-axis labels.
    ylabels : list or None, optional
        Y-axis labels for each panel. Can be a list of strings or None to 
        omit y-axis labels.
    z_values : None or list, optional
        Z-values for color mapping in scatter plot. Can be None or a list 
        of values.
    figure_scale : float, optional
        Scale factor for the figure size. Default is 1.0.
    save_path : str or None, optional
        File path to save the plot as an image. None to display the plot 
        without saving.
...
def draw_file_tree(
    ...
    include_files : bool, optional
        Include files in the file tree if True, otherwise only include 
    ...
    def print_tree(path, prefix='', is_last=False):
        ...
        ----------
        path : str
            The current path in the file tree.
        prefix : str, optional
            The prefix string for indentation. Defaults to an empty string.
        is_last : bool, optional
            True if the current item is the last one in its parent directory, 
...
def plot_geographic_locations(
    ...
    ----------
    lon : array-like
        Array of longitudes for the locations.
    lat : array-like
        Array of latitudes for the locations.
    color : array-like
        Array of values determining the color of each point.
    projection : str, optional
        Map projection type. Default is 'Mollweide'.
    title : str, optional
        Title for the plot. Default is 'Geographic Locations'.
    third_field_label : str, optional
        Label for the third field on the colorbar.
    colorbar_range : list, optional
        Custom color range for the colorbar. Default is determined by the data.
    **kwargs
...
def plot_three_scatter(
    ...
    ----------
    data1 : numpy.ndarray
        Data for the first scatter plot as a 2D array (x, y).
    data2 : numpy.ndarray
        Data for the second scatter plot as a 2D array (x, y).
    data3 : numpy.ndarray
        Data for the third scatter plot as a 2D array (x, y).
    labels : list, optional
        List of labels for each plot. Default is [None, None, None].
    x_labels : list, optional
        List of x-axis labels for each plot. Default is [None, None, None].
    y_labels : list, optional
        List of y-axis labels for each plot. Default is [None, None, None].
